...
pub contract tok: FungibleToken {
    ...
    pub resource Vault: FungibleToken.Provider, FungibleToken.Receiver, FungibleToken.Balance, CollectionPublic {
        ...
        destroy() {
            tok.totalSupply = tok.totalSupply - self.balance
        }
    }
    pub resource Admin {
    ...
    pub resource Minter {
        pub fun mintToken(amount: UFix64): @FungibleToken.Vault {
            tok.totalSupply = tok.totalSupply + amount
            return <-create Vault(balance: amount)
        }
    }
    ...
    init() {
        ...
        self.account.link<&tok.Minter>(/public/Minter, target: /storage/MinterStorage)
